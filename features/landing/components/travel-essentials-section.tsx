"use client";

import { useState } from "react";

import { Cloud, Sun } from "lucide-react";

export function TravelEssentialsSection() {
  const [inrAmount, setInrAmount] = useState<number>(10000);
  const exchangeRate = 83.2; // INR per USD
  const usdEquivalent = (inrAmount / exchangeRate).toFixed(2);

  return (
    <section
      id="travel-tools"
      className="py-16 sm:py-24 lg:py-28 border-t border-zinc-200/80 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-950/60 backdrop-blur-xs transition-colors"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-10 lg:px-16 space-y-10 sm:space-y-12">
        {/* Section Header with Responsive Typography */}
        <div className="space-y-3 max-w-2xl">
          <span className="font-mono text-[11px] font-semibold tracking-widest text-zinc-500 uppercase">
            Contextual Utilities
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-light tracking-tight text-zinc-950 dark:text-zinc-50 leading-[1.15] break-words [text-wrap:balance]">
            Useful when you need it.
            <span className="block sm:inline sm:ml-2">
              <span className="font-serif italic font-normal text-zinc-900 dark:text-zinc-100">
                Quiet when you don't.
              </span>
            </span>
          </h2>
          <p className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 font-normal leading-relaxed pt-1 break-words [text-wrap:balance]">
            No cluttering app switchboards or widget bars. Weather forecasts, live UPI
            currency calculations, Indian rail passes, and emergency numbers live quietly
            alongside your itinerary.
          </p>
        </div>

        {/* 3-Column Card Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Card 1: Weather */}
          <div className="rounded-sm border border-zinc-200 dark:border-zinc-800 bg-[#FAFAF9] dark:bg-zinc-900 p-5 sm:p-6 flex flex-col justify-between space-y-6 shadow-xs">
            <div className="space-y-4">
              <div className="flex items-center justify-between font-mono text-[11px] text-zinc-400 uppercase tracking-wider">
                <span>Weather · Jaipur</span>
                <span>November</span>
              </div>

              <div className="flex items-baseline justify-between pt-1">
                <div>
                  <span className="text-5xl font-light text-zinc-950 dark:text-zinc-50 font-mono tracking-tight">
                    24°
                  </span>
                  <p className="text-xs font-semibold text-zinc-900 dark:text-zinc-100 mt-1">
                    Clear & Pleasant
                  </p>
                  <p className="text-[11px] text-zinc-500 font-mono">High 28° · Low 15°</p>
                </div>
                <Sun className="h-8 w-8 text-amber-500/80 stroke-1" />
              </div>

              {/* 5-Day Mini Strip */}
              <div className="grid grid-cols-5 gap-1 pt-3 text-center border-t border-zinc-200/80 dark:border-zinc-800/80">
                {[
                  { day: "MON", temp: "25°", icon: Sun },
                  { day: "TUE", temp: "24°", icon: Sun },
                  { day: "WED", temp: "23°", icon: Cloud },
                  { day: "THU", temp: "24°", icon: Sun },
                  { day: "FRI", temp: "26°", icon: Sun },
                ].map((d, i) => {
                  const Icon = d.icon;
                  return (
                    <div key={i} className="space-y-1">
                      <span className="font-mono text-[10px] text-zinc-400">{d.day}</span>
                      <Icon className="h-3.5 w-3.5 mx-auto text-amber-500/70" />
                      <span className="font-mono text-[11px] block text-zinc-700 dark:text-zinc-300">
                        {d.temp}
                      </span>
                    </div>
                  );
                })}
              </div>
            </div>

            <div className="pt-3 border-t border-zinc-200/80 dark:border-zinc-800/80 text-[11px] text-zinc-500 leading-relaxed">
              Contextual tip: Afternoon desert sunshine is warm; morning and evening require a light
              linen wrap or jacket.
            </div>
          </div>

          {/* Card 2: Currency Converter */}
          <div className="rounded-sm border border-zinc-200 dark:border-zinc-800 bg-[#FAFAF9] dark:bg-zinc-900 p-5 sm:p-6 flex flex-col justify-between space-y-6 shadow-xs">
            <div className="space-y-4">
              <div className="flex items-center justify-between font-mono text-[11px] text-zinc-400 uppercase tracking-wider">
                <span>Currency Converter</span>
                <span>Interbank FX</span>
              </div>

              <div className="space-y-3">
                <div className="rounded-xs border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-3">
                  <label
                    htmlFor="inr-spend-input"
                    className="block text-[10px] font-mono uppercase text-zinc-400"
                  >
                    You Spend (INR)
                  </label>
                  <div className="flex items-center mt-1">
                    <span className="text-sm font-mono text-zinc-500 mr-1.5 font-semibold">
                      ₹
                    </span>
                    <input
                      id="inr-spend-input"
                      type="number"
                      aria-label="Amount in Indian Rupees"
                      value={inrAmount}
                      onChange={(e) => setInrAmount(Number(e.target.value) || 0)}
                      className="w-full text-xl font-light font-mono text-zinc-950 dark:text-zinc-50 bg-transparent focus:outline-hidden"
                    />
                  </div>
                </div>

                <div className="text-center font-mono text-[11px] text-zinc-400">
                  ≈ 1 USD = 83.20 INR
                </div>

                <div className="rounded-xs border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-3">
                  <span className="block text-[10px] font-mono uppercase text-zinc-400">
                    Equivalent in USD
                  </span>
                  <div className="text-2xl font-light font-mono text-[#2D9BF0] mt-1">
                    ${usdEquivalent}
                  </div>
                </div>
              </div>
            </div>

            <div className="pt-3 border-t border-zinc-200/80 dark:border-zinc-800/80 text-[11px] font-mono text-zinc-400">
              Offline calculation cached without roaming data.
            </div>
          </div>

          {/* Card 3: Country Brief */}
          <div className="rounded-sm border border-zinc-200 dark:border-zinc-800 bg-[#FAFAF9] dark:bg-zinc-900 p-5 sm:p-6 flex flex-col justify-between space-y-4 shadow-xs">
            <div className="space-y-3">
              <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-wider">
                <span className="text-zinc-400">Country Brief · India</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-semibold">
                  Essentials
                </span>
              </div>

              <div className="space-y-3 text-xs leading-relaxed">
                <div>
                  <h5 className="font-semibold text-zinc-900 dark:text-zinc-100">
                    UPI & Digital Payments
                  </h5>
                  <p className="text-[11px] text-zinc-500 mt-0.5">
                    UPI One World enables tourists to scan & pay instantly at street food
                    vendors, auto-rickshaws, and craft bazaars using mobile wallets.
                  </p>
                </div>

                <div>
                  <h5 className="font-semibold text-zinc-900 dark:text-zinc-100">
                    Trains & Transport
                  </h5>
                  <p className="text-[11px] text-zinc-500 mt-0.5">
                    Vande Bharat semi-high speed express connects major hubs. Pre-paid taxi
                    counters and app cabs operate at all international terminals.
                  </p>
                </div>

                <div>
                  <h5 className="font-semibold text-zinc-900 dark:text-zinc-100">
                    Emergency Numbers
                  </h5>
                  <p className="text-[11px] font-mono text-zinc-500 mt-0.5">
                    Emergency: 112 · Tourist Helpline: 1363 (24/7) · Ambulance: 108
                  </p>
                </div>
              </div>
            </div>

            <div className="flex items-center justify-between pt-3 border-t border-zinc-200/80 dark:border-zinc-800/80 text-[10px] font-mono text-zinc-400">
              <span>e-Visa verified</span>
              <span>Updated Season 2026</span>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
